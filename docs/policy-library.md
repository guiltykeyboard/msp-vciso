...
| Tenant role | Read | Create and revise | Approve or retire | Request acknowledgement |
| --- | --- | --- | --- | --- |
| MSP administrator | Yes | Yes | Yes | Yes |
| Customer administrator | Yes | Yes | Yes | Yes |
| MSP analyst | Yes | Yes | No | No |
| Control owner | Yes | Yes | No | No |
| Reviewer | Yes | No | No | No |
| External auditor | Yes | No | No | No |

Administrators can issue recipient-specific acknowledgement links for the exact current approved version. The end user sees only that document, explicitly consents with a typed name, and receives an immutable version-and-hash-pinned receipt without gaining tenant membership. See [end-user policy acknowledgements](policy-acknowledgements.md) for the identity and legal boundaries.
...
- `GET` and `POST /v1/policies/{document_id}/agreements` for acknowledgement administration;
- `DELETE /v1/policy-agreements/{agreement_id}` to revoke a pending request; and
- `POST /v1/policy-agreements:inspect` and `POST /v1/policy-agreements:acknowledge` for the recipient workflow.
